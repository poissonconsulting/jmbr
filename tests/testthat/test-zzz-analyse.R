test_that("analyse character vector", {
...
test_that("analyse vectorized embedded expression", {
  set_analysis_mode("quick")
...
test_that("analyse full nimble notation vectorized embedded nested expression", {
  ...
  model <- model(
    code = {
      bIntercept ~ dnorm(0, sd = 5)
      bYear ~ dnorm(0, sd = .5)

      bHabitatQuality[1] <- 0
      for (i in 2:nHabitatQuality) {
        bHabitatQuality[i] ~ T(dnorm(0, sd = 5.), 0, )
      }

      log_sSiteYear ~ dlnorm(0, sd = 5)
      log_sDensity ~ dt(0, sd = 5, 4.5)
      ...
      for (i in 1:nSite) {
        for (j in 1:nYearFactor) {
          bSiteYear[i, j] ~ dnorm(0, sd = sSiteYear)
        }
      }
